...
React 平台的信号雷达支持按全部目录、科创板、仅本地 CSV、当前标的四种范围批量扫描。科创板范围会从本地 A股目录中筛选 `SSE` 且代码以 `688` 开头的候选，扫描数量上限为 300。
...
默认 Web 平台使用 React + FastAPI，包含数据下载、CSV 查看、策略管理、策略编辑、策略选择回测、组合实验室、信号预览、信号雷达、资金曲线、买卖点、交易记录、信号归因、纸面交易日志、风控和 Mock AI 研究员。信号雷达可以扫描本地目录、科创板、本地 CSV 子集或当前标的，并可显式开启扫描前数据维护。
