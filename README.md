...
Web 操作台启动后会加载该文件，并支持按股票名称或代码搜索选择标的。共享股票选择器会以二级弹窗展示搜索框、推荐索引和自选列表；搜索范围包含本地 A股目录，以及内置的少量美股和数字货币公开标的默认项。系统启动时不再默认选中 `000001`，只有用户自选或已保存的 watchlist 才会成为当前标的。
...
美股和数字货币自选项会保留原始代码并使用独立的托管目录，例如 `data/market/us_stock/NASDAQ/AAPL/` 和 `data/market/crypto/CRYPTO/BTCUSDT/`，避免把非 A股代码补零。当前自动维护的跨市场历史数据先覆盖日线：美股使用 Yahoo chart public endpoint，数字货币使用 Binance Spot Kline public endpoint；分钟级跨市场历史数据仍需后续 provider 配置。
...
- 添加或切换自选股后，React 平台会自动触发最近五年的自选股数据维护；本地 app lifespan scheduler 也会每天执行一次 watchlist 数据维护并记录 `watchlist_data` 自动化运行历史。
...
React 平台提供实时盯盘工作区。该能力复用当前周期和策略选择，后端通过市场行情源适配器轮询最新 K 线，预热策略状态后只对新增 K 线输出事件和策略信号。当前范围是研究提醒和纸面联动前置能力，不会发出实盘委托。

当前实时市场源：

- `a_share`: A股公开分钟 K 线轮询，继续使用 AKShare/public 数据边界。
- `us_stock`: 美股公开实时源，默认注入 `AAPL.NASDAQ`，优先读取 Yahoo chart 无密钥公开接口；接口失败时回退到 deterministic demo K 线，避免盯盘页失去可测事件流。
- `crypto`: 数字货币公开实时源，默认注入 `BTCUSDT.CRYPTO`，优先读取 Binance Spot Kline 公开接口；接口失败时回退到 deterministic demo K 线，避免盯盘页失去可测事件流。

美股 Yahoo chart 源是 best-effort 公共数据接口，数字货币 Binance Kline 源遵循 Binance 公开 Market Data endpoint；二者当前只用于研究提醒和信号预览，不作为交易级行情承诺。
...
当前支持三类监听来源：当前标的、自选股，以及上一轮周末自动扫描保存的周榜优质股 Top 候选。React 页面默认启动自选股和周榜优质股两批股票，后端会按 `code.exchange` 去重并在状态里返回批次统计。该能力适合 `1m/5m/15m/30m/60m` 分钟级准实时盯盘。后续告警历史持久化、OpenClaw 微信/飞书通知和 tick/券商级行情源会在同一事件模型上扩展。
...
当前默认内置策略聚焦缠论核心，只在 React/API 默认策略列表暴露：
...
- `ChanMultiLevelReversalStrategy`: 以日线缠论结构为锚点，使用 `60m/30m` 等低级别做执行确认、提前发现和持仓风险控制。

简单趋势、均值回归、突破、MACD、ATR 等教学模板代码仍保留在源码中供历史测试、CLI 或自定义策略参考，但不再作为系统默认内置策略或默认组合入口展示。所有策略是研究和回测模板，不是收益承诺。
