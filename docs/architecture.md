...
- `research`: 研究信号层，当前将本地 K线转换为分析帧，并生成轻量缠论二买/二卖、缠论结构分型/笔/线段/递归中枢/背驰确认、增强 RSI 超买/超卖/修复/背离和综合评分预览。
...
当前组合层支持多个策略在同一标的、同一 K线序列上的信号聚合。配对交易、统计套利和多因子轮动仍需要多标的数据结构、组合层和仓位分配模块扩展，暂不内置到当前单标的策略引擎。
