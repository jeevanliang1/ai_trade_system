...
- Stock-aware React controls should use the shared watchlist selector and `selectStock` action so `symbol`, `exchange`, `timeframe`, `csv_path`, strategy params, portfolio params, and stale market-derived results stay synchronized.
- Watchlist market-data workflows should use `src/ai_trade_system/data_manager.py` and canonical timeframe-aware paths under `data/market/a_share/{exchange}/{code}/{code}_{exchange}_{timeframe}_{adjust}_latest.csv`; do not reintroduce ad hoc `data/{code}_daily.csv` defaults for stock-aware flows. Product code may expose CLI/API update hooks, but committing machine-local scheduling such as launchd still requires owner confirmation.
...
- OpenClaw/web external research may use search or browser tools, but user-facing fundamental or information-side conclusions must carry verifiable external URL evidence. If OpenClaw returns text without URL-backed sources, record the result as evidence-insufficient instead of presenting it as completed research.
- Saturday automation weekly tasks must complete the whole report chain by default: scan 科创板/创业板 with the default daily-anchor Chan strategy, generate board-level Top10 AI deep-analysis cache, and notify the user through OpenClaw Weixin or Feishu delivery. OpenClaw weekly-result requests should read the cached weekly analysis first and only rerun scanning/research when the cache is missing or explicitly refreshed.
- Signal Radar, automation weekly radar, and Agent/OpenClaw default scan/backtest fallbacks should use the `ChanMultiLevelReversalStrategy` daily-anchor optimized preset (`entry_mode=daily_anchor`, `60m` confirmation, `30m` risk, `max_holding_bars=30`) unless the user explicitly overrides the strategy or score mode.
